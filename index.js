...
function emit$1(host, port, event, payload) {
  ...
  socket.on(CONNECT, function () {
    socket.emit(AUTHENTICATE, { token: _this2._token }).on(AUTHENTICATED, function () {
...
var OFFLINE = StateEnum.values.OFFLINE;
...
var _RunnerNodeStateEnum$$1 = StateEnum.values;
var ONLINE$1 = _RunnerNodeStateEnum$$1.ONLINE;
var MAINTENANCE$1 = _RunnerNodeStateEnum$$1.MAINTENANCE;
...
var ONLINE$2 = StateEnum.values.ONLINE;
var _RunnerQueueStateEnum = RunnerQueueStateEnum.values;
var SCHEDULED = _RunnerQueueStateEnum.SCHEDULED;
...
function getAssigned() {
  ...
  return this.queries.readQueue({ runner: this.id, state: 'Enum::' + SCHEDULED }).then(function (tasks) {
...
var ONLINE$3 = StateEnum.values.ONLINE;
var SCHEDULED$1 = RunnerQueueStateEnum.values.SCHEDULED;
...
function getNextRunner(list, success, fail) {
  ...
  if (idx >= list.length) {
    if (this.state === ONLINE$3) return success(this.info());else return fail(new Error('No runners meet the run requirements'));
  }
  var runner = list[idx];
  idx++;
  if (runner.id === this.id && this.state === ONLINE$3) return success(runner);
  if (!runner.host || !runner.port) return getNextRunner.call(this, list, success, fail, idx);

  return this.emit(runner.host, runner.port, STATUS, undefined, defineProperty({}, STATUS, function (info) {
    if (_.get(info, 'state') !== ONLINE$3) return getNextRunner.call(_this, list, success, fail, idx);
  }), function () {
...
function getOnlineRunner(action, context, queue, socket, requestId) {
  ...
  return this.queries.readRunner({ state: 'Enum::' + ONLINE$3 }).then(function (runners) {
...
function addListeners$1(socket) {
  var _this = this;
  ...
  socket.on(STATUS$1, function (_ref2) {
    var requestId = _ref2.requestId;

    _this.log.trace({ client: client, server: _this._server, event: STATUS$1 }, 'received socket event');
    socket.emit(STATUS$1 + '.' + requestId, _this.info());
  });

  socket.on(SCHEDULE, function (_ref3) {
    var payload = _ref3.payload,
        requestId = _ref3.requestId;

    _this.log.trace({ client: client, server: _this._server, event: SCHEDULE }, 'received socket event');
    event.emit(SCHEDULE, { requestId: requestId, payload: payload, socket: socket });
  });

  socket.on(RUN$2, function (_ref4) {
    var requestId = _ref4.requestId;

    _this.log.trace({ client: client, server: _this._server, event: RUN$2 }, 'received socket event');
    event.emit(RUN$2, { requestId: requestId, socket: socket });
  });

  socket.on(STOP, function (_ref5) {
    var requestId = _ref5.requestId,
        payload = _ref5.payload;

    options = options || {};
    _this.log.trace({ client: client, server: _this._server, event: STOP }, 'received socket event');
    event.emit(STOP, { requestId: requestId, payload: payload, socket: socket });
  });

  socket.on(MAINTENANCE_ENTER, function (_ref6) {
    var requestId = _ref6.requestId,
        payload = _ref6.payload;

    _this.log.trace({ client: client, server: _this._server, event: MAINTENANCE_ENTER }, 'received socket event');
    event.emit(MAINTENANCE_ENTER, { requestId: requestId, payload: payload, socket: socket });
  });

  socket.on(MAINTENANCE_EXIT, function (_ref7) {
    var requestId = _ref7.requestId,
        payload = _ref7.payload;

    _this.log.trace({ client: client, server: _this._server, event: MAINTENANCE_EXIT }, 'received socket event');
    event.emit(MAINTENANCE_EXIT, { requestId: requestId, payload: payload, socket: socket });
  });
}
...
function startListeners$1() {
  ...
  if (!useConnection) {
    ...
    this._io.sockets.on(CONNECTION, socketioJwt.authorize({
      ...
      callback: false
    })).on(AUTHENTICATED$1, function (socket) {
      return addListeners$1.call(_this2, socket);
    });
  } else {
  ...
  // handle local events
  event.on(SCHEDULE, function (_ref8) {
    var requestId = _ref8.requestId,
        payload = _ref8.payload,
        socket = _ref8.socket;

    requestId = !requestId && !socket ? LOCAL_REQUEST : requestId;
    _this2.log.trace({ event: SCHEDULE, requestId: requestId }, 'received local event');
    _this2.schedule(payload, socket, requestId);
  });

  event.on(RUN$2, function (_ref9) {
    var requestId = _ref9.requestId,
        socket = _ref9.socket;

    requestId = !requestId && !socket ? LOCAL_REQUEST : requestId;
    _this2.log.trace({ event: RUN$2, requestId: requestId }, 'received local event');
    _this2.run(socket, requestId);
  });

  event.on(STOP, function (_ref10) {
    var requestId = _ref10.requestId,
        payload = _ref10.payload,
        socket = _ref10.socket;

    requestId = !requestId && !socket ? LOCAL_REQUEST : requestId;
    _this2.log.trace({ event: STOP, requestId: requestId }, 'received local event');
    _this2.stop(payload, socket, requestId);
  });

  event.on(MAINTENANCE_ENTER, function (_ref11) {
    var requestId = _ref11.requestId,
        payload = _ref11.payload,
        socket = _ref11.socket;

    requestId = !requestId && !socket ? LOCAL_REQUEST : requestId;
    _this2.log.trace({ event: MAINTENANCE_ENTER, requestId: requestId }, 'received local event');
    _this2.maintenance(true, payload, socket, requestId);
  });

  event.on(MAINTENANCE_EXIT, function (_ref12) {
    var requestId = _ref12.requestId,
        payload = _ref12.payload,
        socket = _ref12.socket;

    requestId = !requestId && !socket ? LOCAL_REQUEST : requestId;
    _this2.log.trace({ event: MAINTENANCE_EXIT, requestId: requestId }, 'received local event');
...
var OFFLINE$1 = StateEnum.values.OFFLINE;
...
function forceStop(socket, requestId) {
  ...
  socket.on(STOPPING_ACK, function () {
...
// local modules
// constants
var _RunnerNodeStateEnum$ = StateEnum.values;
var ONLINE = _RunnerNodeStateEnum$.ONLINE;
var MAINTENANCE = _RunnerNodeStateEnum$.MAINTENANCE;
...
var YellowjacketServer = function () {
  function YellowjacketServer(backend, options, callback) {
    var _this = this;
    ...
    this.addListeners = addListeners$1.bind(this);
    ...
    .then(function () {
      return _this.queries.getSelf();
    }).then(function (self) {
      _this.id = self.id;
      _this.state = self.state === MAINTENANCE ? MAINTENANCE : ONLINE;
      return self;
    })

    // check in and start the server
    .then(function () {
      return _this.queries.checkIn(true);
    }).then(function () {
...
function scheduleRunner(_ref) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    client.emit(host, port, SCHEDULE$1, { action: action, context: context }, (_client$emit = {}, defineProperty(_client$emit, SCHEDULE_ACCEPT$1, function () {
      resolve('Schedule request accepted');
    }), defineProperty(_client$emit, SCHEDULE_ERROR$2, function (error) {
      reject(error);
...
function stopRunner(_ref2) {
  ...
  return new Promise(function (resolve, reject) {
    client.emit(host, port, STOP$1, {}, defineProperty({}, STOPPING$1, function (socket) {
      socket.emit(STOPPING_ACK$1);
...
function maintenanceRunner(_ref3) {
  ...
  var EVT = exit ? MAINTENANCE_EXIT$1 : MAINTENANCE_ENTER$1;

  return new Promise(function (resolve, reject) {
    var _client$emit3;

    client.emit(host, port, EVT, reason, (_client$emit3 = {}, defineProperty(_client$emit3, MAINTENANCE_OK$1, function () {
      resolve((exit ? 'exit' : 'enter') + 'ed maintenance successfully');
    }), defineProperty(_client$emit3, MAINTENANCE_ERROR$1, function (error) {
      reject(error);
...
var OFFLINE$2 = _StateEnum$values.OFFLINE;
var MAINTENANCE$2 = _StateEnum$values.MAINTENANCE;
...
var checkinRunnerNode = function (backend) {
  return function (source, args, context, info) {
    ...
    var id = args.id,
        state = args.state,
        offlineAfter = args.offlineAfter;

    var collection = backend.getCollection('RunnerNode');

    return collection.get(id).eq(null).branch(r.error('runner not found'), collection.get(id).update({ checkin: r.now(), state: state })).do(function () {
      return collection.filter(function (node) {
        return node('id').ne(id).and(node('state').ne(MAINTENANCE$2)).and(node('checkin').eq(null).or(r.now().sub(node('checkin')).ge(offlineAfter)));
      }).update({ state: OFFLINE$2 });
    }).do(function () {
      return true;
    });
    ...
    return q.type('RunnerNode').update({
      ...
      return q.type('RunnerNode').filter(function (node) {
        return q.value(node).prop('id').ne(args.id).and(q.value(node).prop('state').ne(MAINTENANCE$2).value()).and(q.value(node).prop('checkin').eq(null).or(q.now().sub(q.value(node).prop('checkin').value()).ge(args.offlineAfter).value()).value()).value();
      }).update({ state: OFFLINE$2 }).value();
    }).do(function () {
      return true;
    }).run(_connection);
  };
...
var createRunnerSettings = function (backend) {
  return function (source, args, context, info) {
    ...
    var collection = backend.getCollection('RunnerSettings');

    return collection.count().gt(0).branch(r.error('a settings document has already been created'), collection.insert(args, { returnChanges: true }).pluck('errors', 'first_error', 'changes').do(function (summary) {
      return summary('errors').ne(0).branch(r.error(summary('first_error')), summary.nth(0)('changes')('new_value'));
    })).run(_connection);
  };
};
...
var deleteRunnerSettings = function (backend) {
  return function (source, args, context, info) {
    var _connection = backend._connection;

    var collection = backend.getCollection('RunnerSettings');

    return collection.delete({ returnChanges: true }).pluck('errors', 'first_error').do(function (summary) {
...
var readRunnerSettings = function (backend) {
  return function (source, args, context, info) {
    ...
    return collection.count().eq(0).branch(r.error('a settings document has not been created yet'), collection.nth(0)).run(_connection);
  };
};
...
var updateRunnerSettings = function (backend) {
  return function (source, args, context, info) {
    ...
    return collection.count().eq(0).branch(r.error('a settings document has not been created yet'), collection.nth(0).update(args, { returnChanges: true }).pluck('errors', 'first_error').do(function (summary) {
      return summary('errors').ne(0).branch(r.error(summary('first_error')), collection.nth(0));
    })).run(_connection);
  };
};
