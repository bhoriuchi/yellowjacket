...
export default function (lib, helper) {
  ...
  switch (cmd) {
    case 'list':
      return lib.Runner('{ readRunner { id, host, port, zone { id, name, description, metadata }, state, metadata } }')
        .then((res) => {
    ...
    case 'info':
      ...
      return lib.Runner(`{ readRunner (id: "${id}") { id, host, port, zone { id, name, description, metadata }, state, metadata } }`)
        .then((res) => {
          let nodeInfo = _.get(res, 'data.readRunner[0]')
          if (res.errors) return error(pretty(res.errors))
          if (!nodeInfo) return error(`No node found with ID ${id}`)
          console.log(chalk.blue.bold('Runner Node:'))
